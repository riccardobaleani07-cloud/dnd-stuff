...
public class MaterialDimension extends Dimension {

    // Allowed options for this layer
    private static final EnumSet<TimeFlowModel> ALLOWED_TIME = EnumSet.of(
        TimeFlowModel.DECELERATED, TimeFlowModel.ACCELERATED, TimeFlowModel.NORMAL, TimeFlowModel.FROZEN
    );
    private static final EnumSet<MagicBehavior> ALLOWED_MAGIC = EnumSet.of(
        MagicBehavior.SUPPRESSED, MagicBehavior.NORMAL, MagicBehavior.AMPLIFIED, MagicBehavior.FORBIDDEN
    );
    private static final EnumSet<GravityModel> ALLOWED_GRAVITY_MODELS = EnumSet.of(
        GravityModel.NORMAL, GravityModel.HIGH, GravityModel.LOW
    );


    public MaterialDimension(String name, TimeFlowModel timeFlow,
    ...
    private static <T extends Enum<T>> T checkAllowed(T value, EnumSet<T> allowed, String fieldName) {
        if (!allowed.contains(value)) {
            throw new IllegalArgumentException(
                fieldName + " " + value + " is not allowed in MaterialDimension"
            );
        }
        return value;
    }
